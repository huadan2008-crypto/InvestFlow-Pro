...
if "oid" in params:
    ...
    if sub.empty:
        ...
        if p_match.empty:
            ...
            if is_expired or is_closed:
                ...
                if remaining <= 0:
                    st.warning("⚠️ 额度已报满，新的提交将进入等待名单 (Waitlist)。")
                elif remaining < (p_info['total_capacity'] * 0.2):
                    st.warning(f"🔥 额度极度紧张：仅剩 {format_curr(remaining)}")

                c1, c2, c3 = st.columns(3)
                c1.metric("发行价格", format_curr(p_info['share_price']))
                c2.metric("认购截止日", p_info['expiry_date'])
                c3.metric("个人最高限额", format_curr(p_info['individual_cap']))

                if row['status'] == 'Invited':
                    with st.form("sub_form"):
                        st.subheader("提交您的认购意愿")
                        raw_amt = st.text_input("拟认购金额 (CAD)", value="50,000")
                        amt_val = clean_num(raw_amt)
                        if amt_val > 0: st.success(f"大写预览: {format_curr(amt_val)}")
                        
                        ent = st.text_input("法律实体全称 (Subscription Entity)")
                        phone = st.text_input("联系电话 (Phone Number)")
                        
                        if st.form_submit_button("确认提交"):
                            if amt_val > p_info['individual_cap']:
                                st.error(f"抱歉，单笔认购不得超过上限 {format_curr(p_info['individual_cap'])}")
                            elif amt_val <= 0 or not ent or not phone:
                                st.error("请完整填写认购金额、实体名称及电话号码。")
                            else:
                                df_s.loc[df_s['order_id']==oid, ['amount','entity_name','phone','status']] = [amt_val, ent, phone, "Interested"]
                                df_s.to_csv(SUBS_FILE, index=False); st.success("提交成功！销售经理将尽快审核。"); st.rerun()
                elif row['status'] == 'Interested':
                    st.warning(f"⏱️ 意向已收到，正在审核认购额度：{format_curr(row['amount'])}")
                elif row['status'] == 'Qualified':
                    st.success(f"✅ 认购额度已核准：{format_curr(row['amount'])}")
    ...
    if menu == "Project Manager":
        st.header("🚀 项目生命周期管理")
        with st.expander("➕ 发布全新 PP 融资轮次"):
            q = st.text_input("搜索上市公司 Ticker")
            if q:
                res = yf.Search(q, max_results=5).quotes
                if res:
                    sel = st.selectbox("选择目标公司:", range(len(res)), format_func=lambda x: f"{res[x].get('longname')} ({res[x].get('symbol')})")
                    with st.form("launch_pp"):
                        c1, c2, c3 = st.columns(3)
                        pr = c1.text_input("发行单价", "0.10")
                        cap = c2.text_input("总募集额度", "1,000,000")
                        ind_cap = c3.text_input("单笔认购上限", "200,000")
                        exp_date = st.date_input("项目截止日期", date.today())
                        lock = st.number_input("股份锁定期 (月)", 4)
                        if st.form_submit_button("正式启动项目"):
                            df_p = pd.read_csv(CONFIG_FILE)
                            new_p = {
                                'ticker': res[sel]['symbol'], 'company_name': res[sel]['longname'],
                                'share_price': clean_num(pr), 'total_capacity': clean_num(cap),
                                'individual_cap': clean_num(ind_cap), 'lockup_months': lock,
                                'issue_date': date.today().strftime("%Y-%m-%d"),
                                'expiry_date': exp_date.strftime("%Y-%m-%d"), 'status': 'Active'
                            }
        ...
        for idx, p in df_p.iterrows():
            ...
            p_status = p['status']
            if p_status == 'Active':
                if today_str > str(p['expiry_date']):
                    p_status = 'Expired'
            ...
            with st.container(border=True):
                ...
                col2.write(f"核准进度: {format_curr(q_sum)} / {format_curr(p['total_capacity'])}")
                col2.progress(prog)
                
                st_color = "green" if p_status == "Active" else "red"
                col3.markdown(f"状态: **:{st_color}[{p_status}]**")
                
                # 手动操作逻辑
                if p_status == 'Active':
                    if col3.button("Force Close", key=f"fc_{idx}", help="即刻使所有链接失效并停止收单"):
                    ...
                    csv_bytes = final_df[['name', 'client_email', 'phone', 'entity_name', 'amount']].to_csv(index=False).encode('utf-8-sig')
                    col3.download_button("📥 导出名册", csv_bytes, f"Final_List_{p['ticker']}.csv", "text/csv", key=f"dl_{idx}")

    elif menu == "CRM & Bulk":
        st.header("👥 客户数据库")
        bulk_data = st.text_area("批量粘贴 (格式: Email, Name, Tag)")
        if st.button("开始导入"):
        ...
        if not df_p.empty and not df_c.empty:
            p_options = [f"{r['ticker']} (@{format_curr(r['share_price'])}) - 截止: {r['expiry_date']}" for _, r in df_p.iterrows()]
            sel_p_idx = st.selectbox("选择目标发行轮次", range(len(p_options)), format_func=lambda x: p_options[x])
            target_p = df_p.iloc[sel_p_idx]
            tag_filter = st.selectbox("目标客户群 (Tag)", ["All"] + df_c['tags'].unique().tolist())
            if st.button("生成认购专用链接"):
                targets = df_c if tag_filter == "All" else df_c[df_c['tags'] == tag_filter]
                new_links = [ {'order_id':str(uuid.uuid4())[:8], 'client_email':c['email'], 'phone':"", 'ticker':target_p['ticker'], 'share_price':target_p['share_price'], 'amount':0, 'entity_name':"", 'status':"Invited"} for _, c in targets.iterrows() if not ((df_s['client_email']==c['email']) & (df_s['ticker']==target_p['ticker']) & (df_s['share_price']==target_p['share_price'])).any() ]
                pd.concat([df_s, pd.DataFrame(new_links)]).to_csv(SUBS_FILE, index=False); st.success(f"已成功为 {len(new_links)} 位客户生成链接。")

    elif menu == action_label:
        st.header("⚡ 审批中心")
        pendings = df_s[df_s['status'] == 'Interested']
        if pendings.empty:
            st.info("桌面清爽！暂无待处理的认购申请。")
        else:
            for i, r in pendings.iterrows():
                with st.container(border=True):
                    c1, c2 = st.columns([4,1])
                    c1.write(f"**客户:** {r['client_email']} | **实体:** {r['entity_name']} | **电话:** {r['phone']}")
                    c1.write(f"**意向认购金额:** {format_curr(r['amount'])} (项目: {r['ticker']})")
